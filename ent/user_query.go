...
// QueryShortlinks chains the current query on the "shortlinks" edge.
func (_q *UserQuery) QueryShortlinks() *ShortlinkQuery {
	query := (&ShortlinkClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.To(shortlink.Table, shortlink.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.ShortlinksTable, user.ShortlinksColumn),
		)
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withShortlinks: _q.withShortlinks.Clone(),
...
// WithShortlinks tells the query-builder to eager-load the nodes that are connected to
// the "shortlinks" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithShortlinks(opts ...func(*ShortlinkQuery)) *UserQuery {
	query := (&ShortlinkClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withShortlinks = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withShortlinks; query != nil {
		if err := _q.loadShortlinks(ctx, query, nodes,
			func(n *User) { n.Edges.Shortlinks = []*Shortlink{} },
			func(n *User, e *Shortlink) { n.Edges.Shortlinks = append(n.Edges.Shortlinks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadShortlinks(ctx context.Context, query *ShortlinkQuery, nodes []*User, init func(*User), assign func(*User, *Shortlink)) error {
	...
	query.withFKs = true
	query.Where(predicate.Shortlink(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ShortlinksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_shortlinks
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_shortlinks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_shortlinks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
