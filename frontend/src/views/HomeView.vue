...
const trySubmit = async (fromPaste = false) => {
	...
	const { data, error } = await PUT('/api/v1/shortlinks', {
		...
		params: {
			query: {
				apiKey: isLoggedIn.value ? apiKey.value : undefined
			}
		}
	});
...
<template>
	<div class="LinkCreator">
		...
		<section class="LinkCreator-section">
			...
			<input v-model.trim="destination" placeholder="http://website.tld" type="url"
				@blur="destinationFocused = false" @focus="destinationFocused = true">
		</section>
		...
		<section class="LinkCreator-section">
			...
			<input v-model.trim="shortcode" :placeholder="`Ex: ${exampleShortcode}`" type="text">
		</section>
		...
		<section class="LinkCreator-section">
			...
			<input v-model.trim="duration" placeholder="Ex: 2 days and 6 hours" type="text">
		</section>

		<section class="pt-6 LinkCreator-section">
			<div class="col-span-6 flex items-center justify-between">
				<p v-if="isFirefox" class="LinkCreator-sectionHint">Reading from clipboard is not supported on Firefox.
					Check
					the
					<router-link :to="{ name: 'faq' }">FAQ</router-link>
					for more info.
				</p>
				<p v-else-if="clipboardReadAccess.state.value === 'granted'" class="LinkCreator-sectionHint">Hint: Press
					<key-combo :keys="['ctrl', 'v']" />
					to instantly paste and submit!
				</p>
				<p v-else class="LinkCreator-sectionHint">Hint: Click <a href="#"
						@click.prevent="readClipboardContent">here</a>
					to give the site access to reading your clipboard so you can create links quicker.</p>
