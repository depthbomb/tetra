...
const regenerateKey = async () => {
	...
	if (confirm('There is a two hour wait between generating new a API key.\n\nAre you sure you want to continue?')) {
		...
		const { data, error } = await POST('/api/v1/users/regenerate-api-key', {
			params: {
				query: {
					apiKey: apiKey.value
				}
...
onMounted(async () => {
	const { data, error } = await GET('/api/v1/users/api-key-info', {
		params: {
			query: {
				apiKey: apiKey.value
			}
...
<template>
	<header class="ApiKeyHeader">
		<div class="ApiKeyHeader-input">
			<copy-button :content="apiKey" text="Copy" />
			<input :type="apiKeyHidden ? 'password' : 'text'" :value="apiKey" readonly
				@mouseenter="apiKeyHidden = false" @mouseleave="apiKeyHidden = true">
			<app-button :disabled="!canRegenerateKey" :loading="regeneratingKey" variant="danger"
				@click="regenerateKey">
	...
	<p>As with any API key for any service, do not share it with anyone as it can be used to perform actions on your
		behalf. If you accidentally leaked your API key and need a new one then you can regenerate it. You must wait a
		while
		after regenerating your API key before you can regenerate a new one.</p>
</template>
